...
local NetworkInterfaceCollectionHandler = class("NetworkInterfaceCollectionHandler", RedfishHandler)

function NetworkInterfaceCollectionHandler:get()
	local response = {}
	...
	response["Name"] = "NetworkInterface Collection"
	...
	response = self:oem_extend(response, "query.networkinterface-collection")
	self:set_context("NetworkInterfaceCollection.NetworkInterfaceCollection")
	self:set_type(CONSTANTS.NETWORKINTERFACE_COLLECTION_TYPE)
	self:set_allow_header("GET")
	self:set_response(response)
	self:output()
end

return NetworkInterfaceCollectionHandler
