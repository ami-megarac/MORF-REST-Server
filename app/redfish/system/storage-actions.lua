...
local StorageActionHandler = class("StorageActionHandler", RedfishHandler)
...
function StorageActionHandler:get()
	self:set_header("Allow", "POST")
	self:error_method_not_allowed()
end

function StorageActionHandler:post(url_capture0, url_capture1, url_capture2)
	...
	if space == "Storage" and action == "SetEncryptionKey" then
		if not turbo.util.is_in(request_data.SetEncryptionKeyType, SetEncryptionKey_allowable_vals) then
			self:error_action_parameter_format("SetEncryptionKeyType", request_data.SetEncryptionKeyType)
		end
		local redis_action_key = "Redfish:" .. table.concat(url_segments, ":"):match("(.*):.*") .. ":SetEncryptionKey"
		local action_res = yield(self:get_db():set(redis_action_key, request_data.SetEncryptionKeyType))
